...
void glow::camera::Handle::setLookAt(glm::vec3 const &pos, const glm::vec3 &target, bool keepDistance)
{
    mTarget.target = target;

    glm::vec3 forwardVector = target - pos;
    auto targetDistance = glm::length(forwardVector);
    if (targetDistance < .0001f) // in case target == position
    {
        targetDistance = .0001f;
        forwardVector = glm::vec3(targetDistance, 0, 0);
    }
    mTarget.forward = forwardVector / targetDistance;
    ...
    {
        mTarget.transform.position = pos;
        mTarget.targetDistance = targetDistance;
    }
}
...
void glow::camera::Handle::translateWithTarget(glm::vec3 const& target)
{
    mTarget.target = target;
    mTarget.transform.position = mTarget.target - mTarget.forward * mTarget.targetDistance;
}

void glow::camera::Handle::setPitchInDegree(float pitch)
{
    auto forward = mTarget.forward;
    float radius = glm::sqrt(forward.x * forward.x + forward.z + forward.z);
    forward.y = radius * glm::tan(pitch * glm::pi<float>() / 180.0f);
    mTarget.forward = glm::normalize(forward);
}
