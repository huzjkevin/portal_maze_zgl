...
SharedFramebuffer Framebuffer::createWithDepthAndStencil(const std::string& fragmentName,
                                                         const SharedTexture& color,
                                                         SharedTexture const& depthAndStencil,
                                                         int depthStencilMipmapLevel,
                                                         int depthStencilLayer)
{
    auto fbo = create();
    auto bfbo = fbo->bind();
    bfbo.attachColor({fragmentName, color});
    if (depthAndStencil)
        bfbo.attachDepthStencil(depthAndStencil, depthStencilMipmapLevel, depthStencilLayer);
    if (!bfbo.checkComplete())
        return nullptr;
    return fbo;
}
