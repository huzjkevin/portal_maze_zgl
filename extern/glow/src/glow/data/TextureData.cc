...
SharedTextureData TextureData::createFromRawData(const std::vector<unsigned char>& rawData, unsigned width, unsigned height, ColorSpace colorSpace)
{
    GLOW_ACTION();
    ...
    surface->setData(std::vector<char>(rawData.data(), rawData.data() + rawData.size()));
