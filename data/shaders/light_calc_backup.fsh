...
float orenNayarDiffuse(float albedo, vec3 normal, vec3 lightDir, vec3 viewDir){
    ...
    // CLAMPING
    float Ld = p*(A + (B*max(0.0, VL)*sin(alpha)*clamp(tan(beta), -10.0, 10.0))) / pi; //formular refferred at wikipedia: https://en.wikipedia.org/wiki/Oren%E2%80%93Nayar_reflectance_model
