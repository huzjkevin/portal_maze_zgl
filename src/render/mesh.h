...
class Mesh {
 public:
  ...
  std::vector<T> normals;    /// [xyz] * 3(triangle) * num_faces
  std::vector<T> tangents;   /// [xyz] * 3(triangle) * num_faces
  std::vector<T> facevarying_binormals;  /// [xyz] * 3(triangle) * num_faces
  std::vector<T> uvs;        /// [xy]  * 3(triangle) * num_faces
  std::vector<T>
  ...
  void GetNormal(T Ng[3], T Ns[3], const unsigned int face_idx, const T u, const T v) const {
    ...
    if (normals.size() > 0) {
      ...
      n0[0] = normals[9 * face_idx + 0];
      n0[1] = normals[9 * face_idx + 1];
      n0[2] = normals[9 * face_idx + 2];

      n1[0] = normals[9 * face_idx + 3];
      n1[1] = normals[9 * face_idx + 4];
      n1[2] = normals[9 * face_idx + 5];

      n2[0] = normals[9 * face_idx + 6];
      n2[1] = normals[9 * face_idx + 7];
      n2[2] = normals[9 * face_idx + 8];
  ...
  void GetTexCoord(T tcoord[3], const unsigned int face_idx, const T u, const T v) {
    ...
    if (uvs.size() > 0) {
      ...
      t0[0] = uvs[6 * face_idx + 0];
      t0[1] = uvs[6 * face_idx + 1];
      t0[2] = static_cast<T>(0.0);

      t1[0] = uvs[6 * face_idx + 2];
      t1[1] = uvs[6 * face_idx + 3];
      t1[2] = static_cast<T>(0.0);

      t2[0] = uvs[6 * face_idx + 4];
      t2[1] = uvs[6 * face_idx + 5];
